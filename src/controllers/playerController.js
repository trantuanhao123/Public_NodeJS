...
const {createPlayerService,getWinnerService,updateWalletWinnerService,updateJobCompletionService} =require("../services/playerService")
...
const updateWalletWinner = async (req, res) => {
    try {
        ...
        if (!(name || maNguoiChoi) || tokenBalance === undefined) {
            return res.status(400).json({ message: "Tên người chơi và số dư token là bắt buộc" });
        }
